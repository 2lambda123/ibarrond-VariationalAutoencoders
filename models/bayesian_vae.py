...
class BayesianVAE(object):
    ...
    def sample_from_Z(self, z_mu, z_log_o):
        ...
        """
        return z_mu + tf.exp(z_log_o) * self.get_std_norm_samples([self.L, tf.shape(self.X)[0], self.n_latent])
    ...
    def encode(self, net):
        ...
        with tf.name_scope("encoder"):
            ...
            bias_mean = tf.expand_dims(W_mean[:,0,:], 1)
            mean = tf.matmul(net, W_mean[:,1:,:])
            mean = mean + bias_mean
            
            # latent log_var
            W_log_var = self.sample_from_W(self.mean_W[i_off+1], self.log_var_W[i_off+1])
            bias_log_var = tf.expand_dims(W_log_var[:,0,:], 1)
            log_var = tf.matmul(net, W_log_var[:,1:,:])
            log_var = 0.5 * (log_var + bias_log_var)
    ...
    def get_kl_z(self):
        ...
        return -tf.reduce_mean(tf.reduce_mean(0.5 * tf.reduce_sum( 1.0 + 2.0 * self.z_log_sigma - tf.square(self.z_mu) -
                                   tf.exp(2.0 * self.z_log_sigma), [-1]), [0]))
